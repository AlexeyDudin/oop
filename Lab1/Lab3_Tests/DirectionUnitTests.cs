...
namespace Lab3_Tests
{
    public class Tests
    {
        ...
        public void Positive_GearTest_SetNeutralGear_InStopEngine()
        {
            ...
            //Assert.IsTrue(TrySwitchGear(car, Gear.NEUTRAL), Is.True);
        }
        ...
        public void Negative_GearTest_SetSelfGear_GearBackward()
        {
            ...
            car.SetSpeed(15);
            Assert.False(car.SetGear((int)GearSelector.REVERSE));
        }

        [Test]
        public void Positive_GearTest_ReverseGear_SpeedIsZero()
        {
            const int carSpeed = 0;
            var car = new Car();
            car.TurnOnEngine();
            car.SetSpeed(carSpeed);
            Assert.IsTrue(car.SetGear((int)GearSelector.REVERSE));
        }

        [Test]
        public void Positive_GearTest_FirstGear_SpeedIsZero()
        {
            const int carSpeed = 0;
            var car = new Car();
            car.TurnOnEngine();
